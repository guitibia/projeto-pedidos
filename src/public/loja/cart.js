const Cart = (() => {
  ...
  function getSubtotal(){ return read().reduce((s,i)=>s + (Number(i.price) * i.qty), 0); }
  function addItem(p, qty=1){
    const items = read(); const ex = items.find(i=>i.id===p.id);
    if (ex) ex.qty += qty; else items.push({ id:p.id, name:p.name, price:Number(p.price)||0, image:p.image||null, franchise:p.franchise||'', qty });
    write(items);
  }
